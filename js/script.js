...
$(document).ready(function(){
	...
	$('.a-google-map').each(function(){
		aGoogSrc = $(this).find('iframe').attr('src');
		fixedGoogSrc = aGoogSrc.replace(/iwloc=(.*)&/g,'iwloc=near&');
		$(this).find('iframe').attr('src', fixedGoogSrc);
	});
  ...
  $('.homepage-featured-bar').live('click', function(e) {
    ...
    window.ga('send', 'event', 'Ad', 'Clicked', 'Featured Bar: ' + $(this).data('featured-bar-title'))
  });

  $('.homepage-featured-brewery').live('click', function(e) {
    console.log('Featured brewery clicked: ' + $(this).data('featured-brewery-title'))
    window.ga('send', 'event', 'Ad', 'Clicked', 'Featured Brewery: ' + $(this).data('featured-brewery-title'))
  });
});
...
function runTicker() {
	...
	tickerCounter++;
	var howMany = $('.links-pane a').filter(':first').attr('data-link-number');

	console.log(howMany);
	if ( tickerCounter == howMany ) {
		$('.links-pane').stop().animate({'top':'0px'});
		tickerCounter = 0;
	} else {
		$('.links-pane').stop().animate({'top':'-=60px'});
	}
}
